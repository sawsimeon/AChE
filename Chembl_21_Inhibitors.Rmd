---
title: "New AChE Inhibitors from Chembl21"
...
## Removing Redundancy and curating activity data points
...
curated_data[curated_data==""] <- NA
curated_data_na_removed <- na.omit(curated_data)
df_20 <- curated_data_na_removed
...
## Importing Chembl 21 and Identifying New AChE Inhibitors

```{r, tidy = TRUE}
library(readxl)
df_raw <-  read.csv("AChE_Chembl21.txt", header = TRUE, sep = "\t")
names <- c("CMPD_CHEMBLID", "CANONICAL_SMILES", "STANDARD_TYPE",
           "RELATION", "STANDARD_VALUE", "STANDARD_UNITS",
           "PROTEIN_ACCESSION", "PREF_NAME", 
      "PUBMED_ID",
...
IC50 <- paste0("IC50 Bioactivity data points: ", nrow(df_1))
IC50_omit <- paste0("Omit IC50 with lesser/greater than symbol: ", nrow(df_2))
final <- paste0("Final number of data points of Chembl 21: ", nrow(df_4))

smiles_20 <- df_20$CANONICAL_SMILES

'%!in%' <- function(x,y)!('%in%'(x,y))
new_df <- subset(df_4, CANONICAL_SMILES %!in% smiles_20)



new_inhibitors <- paste0("New Inhibitors of AChE from Chembl 21 (including redundant compounds): ", nrow(new_df))

message <- c(bioactivity, IC50, IC50_omit, final, new_inhibitors)
print(message)
...
non_redundant_compound <- paste0("number of non-redundant compounds: ", 
                                 nrow(non_redundant))
curated <- paste0("number of curated new AChE inhibitors from Chembl 21: ", nrow(curated_data_na_removed))
message <- c(non_redundant_compound, unique, sd_2, combined, curated)
print(message)
